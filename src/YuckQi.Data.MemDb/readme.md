...
- **`UnitOfWork` / `UnitOfWork<TScope>`** &ndash; manages typed `ConcurrentDictionary` instances per entity type; `GetEntities<TDomainEntity, TIdentifier>()` provides direct access to the underlying store
